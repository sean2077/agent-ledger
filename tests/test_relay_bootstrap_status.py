...
def _write_session(parent: Path, slug: str, *, state: str = "active",
    ...
    """Create a minimal pair dir under `parent`. Returns the pair path."""
    sd = parent / slug
...
def test_resolve_skips_session_with_terminal_latest(monkeypatch, tmp_path):
    """Bug fix MAJOR #1: resolve_active_pair must filter through session_is_active."""
    shared = _setup_shared(monkeypatch, tmp_path)
    # pair A: state=active but latest artifact is closed -> NOT truly active
    sa = _write_session(shared, "20260527-stale")
    _publish_terminal_artifact(sa, seq=1, status="closed")
    # pair B: state=active, no artifacts yet -> truly active
    sb = _write_session(shared, "20260527-real")

    env = relay.load_env()
    resolved = relay.resolve_active_pair(env)
...
def test_resolve_skips_session_with_CLOSED_sentinel(monkeypatch, tmp_path):
    """Bug fix MAJOR #1: CLOSED sentinel must filter out stale-active pairs."""
    shared = _setup_shared(monkeypatch, tmp_path)
...
def test_bootstrap_creates_dirs_with_0700_mode(monkeypatch, tmp_path):
    """Bug fix MINOR #5: project + pair dirs must be 0700."""
    ...
    assert (sess.stat().st_mode & 0o777) == 0o700, \
        f"pair dir mode is {oct(sess.stat().st_mode & 0o777)}"
...
def test_bootstrap_creates_flat_session_layout(monkeypatch, tmp_path):
    """D1: bootstrap creates .shared/<pair>/ (no <project> subdir)."""
    shared = _setup_shared(monkeypatch, tmp_path)
...
def test_status_resolves_flat_layout(monkeypatch, tmp_path, capsys):
    """D1: relay status finds pairs directly under .shared/."""
    _setup_shared(monkeypatch, tmp_path)
...
def test_resolve_pair_id_errors_use_pair_vocabulary(monkeypatch, tmp_path):
    """Explicit --pair-id errors should not leak retired session wording."""
    shared = _setup_shared(monkeypatch, tmp_path)
    inactive = _write_session(shared, "20990101-inactive", closed_sentinel=True)
    env = relay.load_env()

    with pytest.raises(SystemExit) as missing:
        relay.resolve_pair(env, "20990101-missing", require_active=True)
    missing_msg = str(missing.value)
    assert "no pair" in missing_msg
    assert "no session" not in missing_msg

    with pytest.raises(SystemExit) as inactive_exc:
        relay.resolve_pair(env, inactive.name, require_active=True)
    inactive_msg = str(inactive_exc.value)
    assert "pair is not active" in inactive_msg
    assert "session is not active" not in inactive_msg


def test_resolve_active_pair_multiple_raises(monkeypatch, tmp_path):
    shared = _setup_shared(monkeypatch, tmp_path)
...
def test_resolve_uses_binding_to_disambiguate_parallel(monkeypatch, tmp_path):
    ...
    assert relay.resolve_active_pair(relay.load_env()).name == a.name
    # rebind to the other -> resolve follows the binding
    assert relay.join_pair(relay.load_env(), shared, other.name) == 0
    assert relay.resolve_active_pair(relay.load_env()).name == other.name
